...
class Layout:
  ...
  def __call__(self, coord) -> int:
    return _apply(self.shape, self.stride, coord)
...
def _apply(shape: NestedTuple, stride: NestedTuple, coord) -> int:
    if isinstance(coord, int):
        s = shape.flatten().int_tuple
        d = stride.flatten().int_tuple
        acc, prod = 0, 1
    ...
    assert len(coord) == shape.rank
    return sum(
        _apply(shape.get_mode(i), stride.get_mode(i), coord[i])
